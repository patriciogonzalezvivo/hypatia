 ...
 * aiaa-2006-6573
 * const double kXKE = 60.0 / sqrt(kXKMPER * kXKMPER * kXKMPER / GeoOps::EARTH_GRAVITATIONAL_CONSTANT);
 * dundee
 * const double kXKE = 7.43669161331734132e-2;
 */
const double kXKE = 60.0 / sqrt(GeoOps::EARTH_EQUATORIAL_RADIUS_KM * GeoOps::EARTH_EQUATORIAL_RADIUS_KM * GeoOps::EARTH_EQUATORIAL_RADIUS_KM / GeoOps::EARTH_GRAVITATIONAL_CONSTANT);
 ...
 */
const double kQOMS2T = pow(((kQ0 - kS0) / GeoOps::EARTH_EQUATORIAL_RADIUS_KM), 4.0);
const double kS = kAE * (1.0 + kS0 / GeoOps::EARTH_EQUATORIAL_RADIUS_KM);
...
void SGP4::setTLE(const TLE& tle) {
    ...
    if (m_elements.getPeriapsis() < 156.0) {
        ...
        qoms24 = pow((120.0 - s4) * kAE / GeoOps::EARTH_EQUATORIAL_RADIUS_KM, 4.0);
        s4 = s4 / GeoOps::EARTH_EQUATORIAL_RADIUS_KM + kAE;
    }
                                         ...
                                         const double sinio) const
{
     ...
     */
    const double x = rk * ux * GeoOps::EARTH_EQUATORIAL_RADIUS_KM;
    const double y = rk * uy * GeoOps::EARTH_EQUATORIAL_RADIUS_KM;
    const double z = rk * uz * GeoOps::EARTH_EQUATORIAL_RADIUS_KM;
    Vector3 position(x, y, z);
    const double xdot = (rdotk * ux + rfdotk * vx) * GeoOps::EARTH_EQUATORIAL_RADIUS_KM / 60.0;
    const double ydot = (rdotk * uy + rfdotk * vy) * GeoOps::EARTH_EQUATORIAL_RADIUS_KM / 60.0;
    const double zdot = (rdotk * uz + rfdotk * vz) * GeoOps::EARTH_EQUATORIAL_RADIUS_KM / 60.0;
